...
struct CK { int cx, cy, cz; bool operator==(const CK& o) const { return cx == o.cx && cy == o.cy && cz == o.cz; } };
struct CKHash { size_t operator()(const CK& k) const noexcept {
...
int main(int argc, char** argv)
{
    ...
    if (NX < 1 || NZ < 1) { fprintf(stderr, "bad NX/NZ\n"); return 1; }
    ...
    for (uint32_t ci = 0; ci < chunkCount; ++ci) {
        ...
        for (uint32_t i = 0; i < m.voxelCount; ++i) {
            ...
            src.push_back(w);
            if (w.x < mn[0]) mn[0] = w.x; if (w.x > mx[0]) mx[0] = w.x;
            if (w.y < mn[1]) mn[1] = w.y; if (w.y > mx[1]) mx[1] = w.y;
            if (w.z < mn[2]) mn[2] = w.z; if (w.z > mx[2]) mx[2] = w.z;
        }
    }
    ...
    for (WV& w : src) { w.x -= mn[0]; w.y -= mn[1]; w.z -= mn[2]; }
    ...
    struct ColKey { int32_t x, z; bool operator==(const ColKey& o) const { return x == o.x && z == o.z; } };
    ...
    struct ColLow { int32_t yLo; uint16_t pal; uint8_t mask; uint8_t ao[3]; };
    ...
    std::vector<WV> skirt;
    {
        ...
        for (auto& kv : perimLow) fillCount += (uint64_t)kv.second.yLo;
    ...
    for (int gz = 0; gz < NZ; ++gz) {
        for (int gx = 0; gx < NX; ++gx) {
            int32_t ox = gx * xSpan;
            int32_t oz = gz * zSpan;
            for (const WV& w : src)   emit(w, ox, oz);
            for (const WV& w : skirt) emit(w, ox, oz);
        }
    ...
    if (paletteCount) fwrite(palette.data(), sizeof(uint32_t), paletteCount, o);
    ...
    vxb += ".vxb";
    std::error_code ec;
    if (fs::remove(vxb, ec)) printf("Removed stale baked sidecar: %s\n", vxb.c_str());
