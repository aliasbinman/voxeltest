...
#define MZ_DEFAULT_WINDOW_BITS 15
    ...
    /* Quickly resets a compressor without having to reallocate anything. Same
     * as calling mz_deflateEnd() followed by
     * mz_deflateInit()/mz_deflateInit2(). */
    int mz_deflateReset(mz_streamp pStream);

    /* mz_deflate() compresses the input to output, consuming as much of the
     * input and producing as much output as possible. */
    /* Parameters: */
     ...
     * and/or output buffers are empty. (Fill up the input buffer or free up
     ...
     * of data that could be generated by deflate(), assuming flush is set to
     ...
     * or -MZ_DEFAULT_WINDOW_BITS (raw deflate). */
    int mz_inflateInit2(mz_streamp pStream, int window_bits);

    /* Quickly resets a compressor without having to reallocate anything. Same
     * as calling mz_inflateEnd() followed by
     * mz_inflateInit()/mz_inflateInit2(). */
    int mz_inflateReset(mz_streamp pStream);

    /* Decompresses the input stream to the output, consuming only as much of
     * the input as needed, and writing as much to the output as possible. */
    /* Parameters: */
    /*   pStream is the stream to read from and write to. You must
     * initialize/update the next_in, avail_in, next_out, and avail_out members.
     */
    ...
    /*   MZ_PARAM_ERROR if one of the parameters is invalid. */
    /*   MZ_BUF_ERROR if no forward progress is possible because the input
     * buffer is empty but the inflater needs more input to continue, or if the
     * output buffer is not large enough. Call mz_inflate() again */
    /*   with more input data, or with more room in the output buffer (except
...
#define TDEFL_LESS_MEMORY 0
     ...
     * probes per dictionary search. 0=Huffman only, 1=Huffman+LZ (fastest/crap
     ...
     * initialization time to the minimum, but the output may vary from run to
     * run given the same input (depending on the contents of memory). */
    /* TDEFL_RLE_MATCHES: Only look for RLE matches (matches with a distance of
     * 1) */
    ...
    /* On entry: */
    /*  pImage, w, h, and num_chans describe the image to compress. num_chans
     * may be 1, 2, 3, or 4.
     */
    ...
    /*  The caller must mz_free() the returned heap block (which will typically
     * be larger than *pLen_out) when it's no longer needed. */
...
enum
{
     ...
     * helper functions aren't flexible enough. The low-level functions don't
    ...
    /* There is no corresponding deinit() function because the tdefl API's do
    ...
    /* level may range from [0,10] (where 10 is absolute max compression, but
     * may be much slower on some files) */
...
extern "C"
{
     ...
     * and ends with an adler32 checksum (it's a valid zlib stream). Otherwise,
    ...
    /* On entry: */
    /*  pSrc_buf, src_buf_len: Pointer and size of the Deflate or zlib source
     * data to decompress. */
    /* On return: */
...
#define TINFL_DECOMPRESS_MEM_TO_MEM_FAILED ((size_t)(-1))
     ...
     * internal 32KB buffer, and a user provided callback function will be
...
#define TINFL_LZ_DICT_SIZE 32768
    ...
    typedef enum
    {
        ...
        /* is probably corrupted. If you call the inflator again with more bytes
           it'll try to continue processing the input but this is a BAD sign
        ...
        /* This flags indicate the inflator is finished but the adler32 check of
           the uncompressed data didn't match. If you call it again it'll return
           TINFL_STATUS_DONE. */
        ...
        /* flag on the next call if you don't have any more source data. If the
           source data was somehow corrupted it's also possible (but unlikely)
        ...
        /* Note if the source compressed data was corrupted it's possible for
           the inflator to return a lot of uncompressed data to the caller. I've
...
extern "C"
{
    ...
    enum
    {
        /* Note: These enums can be reduced as needed to save memory or stack
           space - they are pretty conservative. */
    ...
    typedef struct
    {
         ...
         * currently only support up to UINT_MAX or less bytes in the central
         * dir. */
    ...
    typedef enum
    {
        ...
        MZ_ZIP_FLAG_VALIDATE_LOCATE_FILE_FLAG =
            0x1000, /* if enabled, mz_zip_reader_locate_file() will be called on
                       each file as its validated to ensure the func finds the
                       file in the central dir (intended for testing) */
        MZ_ZIP_FLAG_VALIDATE_HEADERS_ONLY =
            0x2000, /* validate the local headers, but don't decompress
                       the entire file and check the crc32 */
        MZ_ZIP_FLAG_WRITE_ZIP64 =
            0x4000, /* always use the zip64 file format, instead of the original
    ...
    /* file_start_ofs is the file offset where the archive actually begins, or
     * 0. */
     ...
     * directory marker, or if it contained any zip64 extended file information
...
/* TODO */
     ...
     * zip64 extended information block, and the optional descriptor following
    ...
    /* Validates an entire archive by calling mz_zip_validate_file() on each
     * file. */
     ...
     * archive's filename so it can be reopened for writing. If the file can't
     * be reopened, mz_zip_reader_end() will be called.
     */
    /* For archives opened using mz_zip_reader_init_mem, the memory block must
     * be growable using the realloc callback (which defaults to realloc unless
    ...
    /* Note: In-place archive modification is not recommended unless you know
     * what you're doing, because if execution stops or something goes wrong
     * before */
    ...
    /* To add a directory entry, call this method with an archive name ending in
     * a forwardslash with an empty buffer. */
     ...
     * field, and optionally supply the function with already compressed data.
     */
    ...
    /* Finalizes the archive by writing the central directory records followed
     * by the end of central directory record. */
    /* After an archive is finalized, the only valid call on the mz_zip_archive
     * struct is mz_zip_writer_end(). */
    /* An archive must be manually finalized by calling this function for it to
     * be valid. */
    ...
    /* Ends archive writing, freeing all allocations, and closing the output
     * file if mz_zip_writer_init_file() was used. */
    ...
    /* Note this is NOT a fully safe operation. If it crashes or dies in some
     * way your archive can be left in a screwed up state (without a central
     * directory). */
    /* level_and_flags - compression level (0-10, see MZ_BEST_SPEED,
     * MZ_BEST_COMPRESSION, etc.) logically OR'd with zero or more mz_zip_flags,
     * or just set to MZ_DEFAULT_COMPRESSION. */
    /* TODO: Perhaps add an option to leave the existing central dir in place in
     * case the add dies? We could then truncate the file (so the old central
